...
def predict_dpi(x, s):
    ...
    for i in range(len(s)):
        y_i = s[i, len(x)]
        x_i = s[i, :len(x)]
        exp = decimal.Decimal(-0.25 * norm(x - x_i) ** 2).exp()
        num += decimal.Decimal(y_i) * exp
        den += exp
...
def evaluate_performance_with_htime(prices, dps, t, step):
    """Use the third time period to evaluate the performance of the algorithm.
    ...
    all_hold_time = []
    hold_time = 0
    buy_prize, sell_price, profit = 0, 0, 0
    old_position = 0
    for i in range(720, len(prices) - 1, step):
        ...
        if dps[i - 720] < -t and position >= 0:
            position -= 1
            bank_balance += prices[i]
            history.append(("sell", prices[i]))
            all_hold_time.append(hold_time)
            hold_time = 0
            if position == -1:
                sell_price = prices[i]
        ...
        if position == 0:
            if old_position == 1:
                profit += prices[i] - buy_prize
            elif old_position == -1:
                profit += sell_price - prices[i]

        cumulative_profit_series.append(profit)
        prices_series.append(prices[i])

    # sell what you bought
    if position == 1:
        bank_balance += prices[len(prices) - 1]
        history.append(("sell", prices[len(prices) - 1]))
        if hold_time != 0 : all_hold_time.append(hold_time)
        cumulative_profit_series.append(profit+prices[len(prices) - 1] - buy_prize)
    # pay back what you borrowed
    elif position == -1:
        bank_balance -= prices[len(prices) - 1]
        history.append(("buy", prices[len(prices) - 1]))
        cumulative_profit_series.append(profit+sell_price - prices[len(prices) - 1])
    
    prices_series.append(prices[len(prices) - 1])

    return {"total_pnl":bank_balance, "pnl":bank_balance/len(history), "cul_p":cumulative_profit_series, "prices_series":prices_series, 
            "avg_hold_time":np.average(all_hold_time), "trades":len(history)}
